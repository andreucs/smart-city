...
def plot_test_time_series(
    ...
    test_df = df.loc[is_may_2025].copy()
    ...
    feature_cols = [c for c in test_df.columns if c not in ('timestamp', TARGET)]
    ...
    for idx, (ax, sid) in enumerate(zip(axes, station_ids)):
        sd = test_df[test_df['id'] == sid].sort_values('timestamp')
        x = sd['timestamp']
        y_true = sd[TARGET]
        y_pred = sd['predicted']
